...
# Add app parent directory to Python path to import shared_store
...
def get_latest(api_hostname: Optional[str] = None,
    ...
    """Fetch latest stream info and map to APL fields.

    Priority 1: Read directly from shared_store in memory (avoiding HTTP overhead & latency).
    Priority 2: Fallback to local HTTP request to the Flask API.
    """
    global info

    # PRIORITY 1: Read directly from the in-memory shared_store (no cache/no HTTP latency)
    try:
        import shared_store
        if shared_store._store and shared_store._store.get('streamUrl'):
            ...
            # Rewrite FLAC stream URLs to MP3 for better device compatibility
    ...
    # PRIORITY 2: FALLBACK to original local loopback HTTP request behavior
    ...
    try:
        ...
        with urllib.request.urlopen(req, timeout=timeout) as resp:
            ...
            # Rewrite FLAC stream URLs to MP3 for compatibility
            if stream_url and isinstance(stream_url, str):
                try:
                    stream_url = re.sub(r'(?i)\.flac(?=$|\?)', '.mp3', stream_url)
                except Exception:
